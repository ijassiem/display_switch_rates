...
# ===================================================================================================================
    ...
    try:
        ...
        logger.info('Connection accepted from %s port %s', address, PORT)
        while True:
            matrix = get_discard(switch_dict, ssh_list, matrix)  # get updated matrix
            msg = msg_formatter(matrix)  # format msg for sending/tx
            #msg = msg_formatter('test message 1')  # format msg for sending/tx
            clientsocket.send(msg)  # send message
            logger.info('Message sent to client. Size %s bytes.', str(sys.getsizeof(msg)))
