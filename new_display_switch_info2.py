...
if __name__ == '__main__':
    ...
    parser.add_option('-d', '--display', type=str, default='leaves',  # spines or leaves
    ...
    def get_rates(switch_dict, ssh_list):
        def get_discard(switch_dict, ssh_list):
            ...
            while not good_output and timeout > 0:
                try:
                    for output in all_output:
                        # IPython.embed()
                        sw_name_idx = [i for i, s in enumerate(output) if 'CBFSW' in s][
                            0]  # [0] is index of desired element in list comprehension. sw_name_idx is index of element containing CBFSW
                        sw_name = output[sw_name_idx].split(' ')[0].split('-')[
                            -1]  # extracts eg. L28 from string: CBFSW-L28 [standalone: master] > terminal type dumb'
                        for i, line in enumerate(output):
                            if line.startswith('status'):
                                sw_status = line.split(':')[
                                    1]  # splits into list'  60 seconds ingress rate: 24864 bits/sec, 3108 bytes/sec, 9 packets/sec'
                            ...
                            if line.startswith('  60 seconds ingress'):
                                in_rate = line.split(',')[
                                    1]  # splits into list'  60 seconds ingress rate: 24864 bits/sec, 3108 bytes/sec, 9 packets/sec'
                                in_rate = [int(i) for i in in_rate.split() if i.isdigit()][
                                    0]  # extract digits, stored as long integer
                            if line.startswith('  60 seconds egress'):
                                egr_rate = line.split(',')[
                                    1]  # splits into list'  60 seconds egress rate: 24864 bits/sec, 3108 bytes/sec, 9 packets/sec'
                                egr_rate = [int(i) for i in egr_rate.split() if i.isdigit()][
                                    0]  # extract digits, stored as long integer
            ...
            matrix = [[[0 for x in range(cols)] for y in range(lines)] for z in
                      range(2)]  # creates matrix, a 3-D list of zeros, lines x cols
